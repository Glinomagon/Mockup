...
func _ready() -> void:
	...
	var default_size_option_index: int = get_size_option_id(GuiAutoload.current_resolution)
...
func get_size_option_id(text: String) -> int:
	for i: int in range(size_option.item_count):
...
func _on_size_option_item_selected(index: int) -> void:
	var res_string: String = size_option.get_item_text(index)
	GuiAutoload.change_window_resolution(res_string)

func _on_window_option_item_selected(index: int) -> void:
	var mode_string: String = window_option.get_item_text(index)
	GuiAutoload.change_window_mode(mode_string)

func _on_resolution_change(new_res: String) -> void:
